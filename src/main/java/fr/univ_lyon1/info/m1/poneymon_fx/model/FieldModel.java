...
public class FieldModel implements Model, Serializable {
    public static final CollisionManager COLLISIONMANAGER =
            new CollisionManager();
    private static final int NB_LAPS = 5;
    // State of the poneys. True : AI, False : Human
    private static final boolean[] isAi =
    ...
    public FieldModel(final int nbParticipants, final boolean soloGame) {
        ...
        if (soloGame) {
            ...
            // Initializing participants and their specific lanes except for
            // the first one.
            for (int i = 1; i < participants.length; i++) {
                participants[i] =
                        new PoneyModel(PoneyModel.getColor(i), i, true,
                                NB_LAPS);
    ...
    public FieldModel(final int nbParticipants) {
        ...
        for (int i = 0; i < participants.length; i++) {
            participants[i] =
                    new PoneyModel(PoneyModel.getColor(i), i, isAi[i], NB_LAPS);
            participants[i].addSelfToTransforms();
            lanes[i] = new LaneEntityModel(i, participants[i]);
            for (FixedEntityModel fe : levelsBuild.getFixedEntities()) {
                if (fe.getRow() == i) {
    ...
    public void setParticipant(String entityType, String color, int indice) {
        ...
        switch (entityType) {
            case "pony":
                participants[indice] =
                        new PoneyModel(color, indice, false, NB_LAPS);
                participants[indice].addSelfToTransforms();
                lanes[indice] =
                        new LaneEntityModel(indice, participants[indice]);
                for (FixedEntityModel fe : levelsBuild.getFixedEntities()) {
                    if (fe.getRow() == indice) {
